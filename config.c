...
void cargarConfig(){
    ...
    strcpy(config.adminUser, "admin");
    strcpy(config.adminPass, "admin");

    fscanf(f, "AdminUser=%49s\n", config.adminUser);
    fscanf(f, "AdminPass=%49s\n", config.adminPass);
    fscanf(f, "HorarioApertura=%9s\n", config.horarioApertura);
    fscanf(f, "HorarioCierre=%9s\n", config.horarioCierre);
    fscanf(f, "DuracionPorDefecto=%d\n", &config.duracionPorDefecto);
    fscanf(f, "AntelacionMaxima=%d\n", &config.antelacionMaxima);
    fscanf(f, "RutaBD=%99s\n", config.rutaBD);
    fscanf(f, "RutaLogs=%99s\n", config.rutaLogs);
    fscanf(f, "AdminUser=%49s\n", config.adminUser);
    fscanf(f, "AdminPass=%49s\n", config.adminPass);
...
void mostrarConfig(){
    printf("\n      CONFIGURACION\n"); //el espacio esta puesto a proposito para que quede por la mitad
    printf("Horario apertura: %s\n", config.horarioApertura);
    printf("Horario cierre: %s\n", config.horarioCierre);
    printf("Duracion por defecto: %d\n", config.duracionPorDefecto);
    printf("Antelacion maxima: %d\n", config.antelacionMaxima);
    printf("Ruta BD: %s\n", config.rutaBD);
    printf("Ruta logs: %s\n", config.rutaLogs);
